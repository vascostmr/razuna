...
<cffunction hint="Add a New Folder" name="add" output="true" returntype="string">
	...
	<cfelse>
		...
		<cfset arguments.thestruct.link_folder_level = 1>
		<!--- Add the folder --->
		<cfthread intstruct="#arguments.thestruct#">
			...
			<cfif thesubdirs.recordcount NEQ 0>
				...
				<cfif attributes.intstruct.rid EQ 0>
					<cfset attributes.intstruct.rid = attributes.intstruct.newfolderid>
				</cfif>
				<cfset attributes.intstruct.link_folder_level = attributes.intstruct.link_folder_level + 1>
				<!--- Call function --->
				<!--- <cfthread intstruct="#arguments.thestruct#"> --->
...
<cffunction name="fnew_detail" output="true" returntype="string" access="public">
	...
	folder_create_time, folder_change_time
	<cfif arguments.thestruct.link_path NEQ ''>,link_path</cfif> ,host_id
...
<!--- Sync the folder assets---> 
<cffunction name="sync_folder_assets" output="false">
	<cfargument name="thestruct" required="yes" type="struct">
	<!--- param --->
	<cfparam name="arguments.thestruct.langcount" default="1" />
	...
	<cfloop list="#folderids#" index="i">
			...
			<cfquery name="qry_detail" datasource="#application.razuna.datasource#">
				SELECT  /* #variables.cachetoken#getsynfolderdetails */ 
					img_id AS ID, img_filename AS filename, img_filename_org AS org_name, folder_id_r, link_kind, link_path_url, path_to_asset, 'img' as type,cloud_url_org
				FROM #session.hostdbprefix#images
				WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
				AND folder_id_r = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#i#">
				UNION ALL
				SELECT vid_id AS ID, vid_filename AS filename, vid_name_org AS org_name, folder_id_r, link_kind, link_path_url, path_to_asset, 'vid' as type,cloud_url_org
				FROM #session.hostdbprefix#videos
				WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
				AND folder_id_r = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#i#">
				UNION ALL
				SELECT aud_id AS ID, aud_name AS filename, aud_name_org AS org_name, folder_id_r, link_kind, link_path_url, path_to_asset, 'aud' as type,cloud_url_org
				FROM #session.hostdbprefix#audios
				WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
				AND folder_id_r = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#i#">
				UNION ALL
				SELECT file_id AS ID, file_name AS filename, file_name_org AS org_name, folder_id_r, link_kind, link_path_url, path_to_asset, 'doc' as type,cloud_url_org
				FROM #session.hostdbprefix#files
				WHERE host_id = <cfqueryparam cfsqltype="cf_sql_numeric" value="#session.hostid#">
				AND folder_id_r = <cfqueryparam CFSQLType="CF_SQL_VARCHAR" value="#i#">
			</cfquery>
					...
					<cfif !listFindNoCase(valueList(arguments.thestruct.thefiles.name),qry_detail.org_name)>
						<cfif qry_detail.link_kind EQ 'lan'>
							...
							<cfif qry_detail.type EQ 'img'>
								<cfinvoke component="images" method="removeimage" thestruct="#attributes.instruct.thestruct#" />
							<cfelseif qry_detail.type EQ 'vid'>	
								<cfinvoke component="videos" method="removevideo" thestruct="#attributes.instruct.thestruct#" />
							<cfelseif qry_detail.type EQ 'aud'>	
								<cfinvoke component="audios" method="removeaudio" thestruct="#attributes.instruct.thestruct#" />
							<cfelseif qry_detail.type EQ 'doc'>	
								<cfinvoke component="files" method="removefile" thestruct="#attributes.instruct.thestruct#" />
							</cfif> 
						</cfif>
					</cfif>
